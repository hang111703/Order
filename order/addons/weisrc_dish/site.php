...
//      为了获得网页标题添加此参数 －2018/06/15
        ...
        //以下是预订时填写的用户地址地址 -add 2018-06-01
        if ($storeid != 0 && $mode == 3) {  //预订
            $backurl = $this->createMobileUrl('reservationdetail',array('storeid' => $storeid, 'mode' => $mode, 'selectdate' => $selectdate, 'timeid' => $timeid, 'rtype' => $rtype),
            true);
        }
...
//                $content .= '备注:' . $order['remark'] . $huanhang;
            ...
            if ($value['is_print_all'] == 1) {
                ...
                foreach ($order['goods'] as $v) {
                    ...
                    $content1 .= "" . $goodsname . ' ' . $goodsid[$v['id']]['total'] . $v['unitname'] . ' ' . number_format($money, 2) . "元" . $huanhang;
                }
                ...
                foreach ($order['goods'] as $v) {
                    ...
                    if ($pos == 0) {
                        ...
                        if (!empty($goodsid[$v['id']]['optionname'])) {
                            $content1 .= '^B2' . '[' . $goodsid[$v['id']]['optionname'] . ']' . $huanhang;
                        }
                        $content1 .= '^B2数量:' . $goodsid[$v['id']]['total'] . $v['unitname'] . $huanhang;
                    } else {
                        $content1 .= '<B>' . $goodsname . '</B>' . $huanhang;
                        if (!empty($goodsid[$v['id']]['optionname'])) {
                            $content1 .= '<B>' . '[' . $goodsid[$v['id']]['optionname'] . ']' . '</B>'  . $huanhang;
                        }
    ...
    public function _yilianyunSendFreeMessage($orderid = 0, $position_type = 0)
    {
        ...
        foreach ($settings as $item => $value) {
            ...
            $order['goods'] = $goods;
            if ($value['is_print_all'] == 1) {if ($value['type'] == 'yilianyun') {
                    $print_order_data = array(
                        'weid' => $weid,
                        'orderid' => $orderid,
                        'print_usr' => $value['print_usr'],
                ...
                foreach ($order['goods'] as $v) {
                    ...
                    $money = $goodsid[$v['id']]['price'];
                    $money = $money * $goodsid[$v['id']]['total'];
                    $content1 .= '<tr><td>' . $goodsname . '</td><td>' . $goodsid[$v['id']]['total'] .
                        $v['unitname'] . '</td><td>' .
        ...
        foreach ($settings as $item => $value) {
            ...
            if ($value['is_print_all'] == 1) {
                ...
                foreach ($order['goods'] as $v) {
                    ...
                    $money = $goodsid[$v['id']]['price'];
                    $money = $money * $goodsid[$v['id']]['total'];
                    $content1 .= $goodsname . ' ' . $goodsid[$v['id']]['total'] . $v['unitname'] . ' ' . number_format($money, 2) . "元\n";
                }
                if (!empty($value['qrcode_url']) && !empty($value['qrcode_status'])) {
                    $print_bottom .= "<QR>" . $value['qrcode_url'] . "</QR>";
                }
...
//                    }
    ...
    public function doWebRetreat()
    {
        ...
        $orderid = intval($_GPC['orderid']);
        $goodsid = substr($_GPC['goodsid'],0,strpos($_GPC['goodsid'],'plus'));  //商品的序号
        $goodsnum = intval($_GPC['goodsnum']);//商品数量
        $optionid = substr($_GPC['goodsid'],strpos($_GPC['goodsid'],'plus') + 4);   //商品的规格序号
...
DESC LIMIT 1", array(':weid' => $this->_weid, ':orderid' => $orderid));
        ...
        if ($optionid == false) {
            ...
            $item = pdo_fetch("SELECT * FROM " . tablename($this->table_order_goods) . " WHERE weid=:weid AND goodsid=:goodsid AND orderid=:orderid AND optionid=:optionid
                ORDER by id DESC LIMIT 1", array(':weid' => $this->_weid, ':goodsid' => $goodsid, ':orderid' => $orderid, ':optionid' => $optionid));
        }
...
DESC LIMIT 1", array(':weid' => $this->_weid, ':goodsid' => $item['goodsid']));
        ...
        if ($goods['counts'] == -1 || $order['dining_mode'] == 3) {
            //不考虑每日库存或者预订,退菜仅造成总销量减少
            ...
            if ($order['dateline'] >= $start && $order['dateline'] <= $end ) {
                //当天的退菜造成总销量减少和每日库存增加
                pdo_update($this->table_goods, array('today_counts' => $goods['today_counts'] - $goodsnum, 'sales' => $goods['sales'] - $goodsnum), array('weid' =>
                    $this->_weid, 'id' => $goodsid));
            } else {
                //不是当天的退菜，只造成总销量减少
                pdo_update($this->table_goods, array('sales' => $goods['sales'] - $goodsnum), array('weid' => $this->_weid, 'id' => $goodsid));
            }
